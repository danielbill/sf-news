...
列表页为 SPA，但 httpx 拿到的初始 HTML 中已包含文章链接 a[href*="/p/"]。
列表页无明确时间元素，需要从详情页获取 publish_time。
"""
...
async def parse(response: Response, source_config: Dict[str, Any], client: AsyncClient = None, limit: int = 20) -> List[Article]:
    ...
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            # 多取一些以应对详情页时间解析失败被跳过的情况
            if len(candidates) >= min(limit * 3, _MAX_DETAIL_FETCHES):
                break

        # 列表页无时间，需要逐个抓取详情页补全 publish_time
        for title, url in candidates:
            ...
            try:
                ...
                articles.append(Article(
                    title=title,
                    url=url,
                    source=SourceType.KR36_AI,
                    publish_time=publish_time,
                ))
            except Exception as e:
                print(f"[36kr-ai] Error fetching detail {url}: {e}")
                continue

    except Exception as e:
        print(f"[36kr-ai] Error: {e}")
...
async def _fetch_publish_time(url: str, client: AsyncClient) -> datetime | None:
    ...
    36氪详情页时间常见形态：
        - <time> 标签或 .article-time / .title-icon-article time
    ...
    try:
        ...
        # 3) 常见 class
        for cls in ("article-time", "title-icon-article", "kr-flow-time"):
        ...
        print(f"[36kr-ai] Error parsing time from {url}: {e}")
...
async def fetch_content(url: str, client: AsyncClient) -> str:
    ...
    try:
        ...
        content_div = soup.find("div", class_="article-content") or \
                      soup.find("div", class_="articleDetailContent") or \
                      soup.find("div", class_="kr-flow-article-body") or \
                      soup.find("article")
