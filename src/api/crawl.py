        ...
        try:
            ...
            if not db.article_exists(article.url):
                ...
                if save_content and article.content:
                    ...
                    article_date = article.timestamp.date() if hasattr(article.timestamp, 'date') else date.today()
                    filename = article.title[:50].replace('/', '-').replace('\\', '-').replace(':', '-').replace('*', '-').replace('?', '-').replace('"', '-').replace('<', '-').replace('>', '-').replace('|', '-')
...
@router.post("/trigger")
async def trigger_crawl(source_id: str = None) -> Dict[str, Any]:
    """手动触发抓取
...
async def get_cache_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "message": "success",
        "data": {
            "cache_date": str(url_cache.cache_date),
            "url_count": url_cache.count
        }
    }
